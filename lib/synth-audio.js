 ...
 */
async function synthAudio(client, logger, stats, {vendor, language, voice, gender, text, salt, ttsCacheExpires }) {
  let audioBuffer;
  logger = logger || noopLogger;
  ttsCacheExpires = ttsCacheExpires || EXPIRES;
  ...
  if (result) {
    ...
    client.expireAsync(key, ttsCacheExpires).catch((err) => logger.info(err, 'Error setting expires'));
  }
  if (!result) {
    ...
    client.setexAsync(key, ttsCacheExpires, audioBuffer.toString('base64'))
